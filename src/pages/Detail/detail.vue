        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl
                v-for="choose in spuSaleAttrList"
                :key="choose.id"
              >
                <dt class="title">{{choose.saleAttrName}}</dt>
                <dd
                  changepirce="0"
                  v-for="spuChoose in choose.spuSaleAttrValueList"
                  :key="spuChoose.id"
                  @click="isChecked(spuChoose,choose)"
                  :class="{active:spuChoose.isChecked == '1'}"
                >{{spuChoose.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
...
export default {
  ...
  methods: {
    isChecked(spuChoose, choose) {
      for (let i = 0; i < choose.spuSaleAttrValueList.length; i++) {
        choose.spuSaleAttrValueList[i].isChecked = "0";
      }
      spuChoose.isChecked = "1";
    },
    changeInput(event) {
      //如果输入的为非数字字符串
    ...
    async addShopCar() {
      //加入购车发请求 前台将参数带给服务器
      //服务器写入数据成功,不返回数据 只是返回code=200 代表操作成功
      //因为服务器没有返回其余数据 因此不需要在vuex中储存数据
      ...
      try {
        ...
        //在回话存储中存储信息,来实现AddCartSuccess组件中的使用 但是储存在回话储存器中的只能是字符串 不能是对象
